...
- **Build system2**
  ...
  2. `cd System2-master`
  3. `make SMSDK=$BUILD_DIR/sourcemod-master OPENSSL=$BUILD_DIR/openssl-1.0.2a ZLIB=$BUILD_DIR/zlib-1.2.8 CURL=$BUILD_DIR/curl-7.46.0`
...
- **Build libcurl**
  ...
  6. Move the `libcurl_a.lib` and the `include` folder from `curl-7.46.0/builds/libcurl-vc12-x86-release-static-ipv6-sspi-winssl` to some folder
